...
public class UserResource {
	...
	/**
	 * GET /user-activation-stats : Get total and activated user statistics.
	 *
	 * This API retrieves user activation statistics from the service layer. If the
	 * statistics object is null
	 * UserStateNotFoundException is thrown.
	 *
	 * @return ResponseEntity containing UserActivationStatsDTO with HTTP 200 (OK)
	 * @throws UserStateNotFoundException if no users are found in the system
	 */
	@GetMapping("/user-activation-stats")
	@PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
	public ResponseEntity<UserActivationStatsDTO> getUserActivationStats() {

	    UserActivationStatsDTO stats = userService.getUserActivationStats();

	    if (stats == null) {
	        throw new UserStateNotFoundException("No users found in the system");
	    }
